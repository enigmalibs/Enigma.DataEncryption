...
key pair — and call `EncryptAsync` or `DecryptAsync` on it with an input stream, an output stream and the
credential. Each of the four derives or transports a 32-byte data key, then encrypts the payload with a
...
- [File operations](file-operations.md) — `DataEncryptionFileExtensions`: the twelve file-path wrappers and
